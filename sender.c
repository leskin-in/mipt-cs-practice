...
void *send_thread(void *args_raw)
{
     ...
     while (true)
     {
          ...
          size_t at_step_read_bytes = fread(buff, 1, size_to_read, file);
          ...
          {
               printf("Failed to send data via TCP: %s [%d]", strerror(errno), errno);
               break;
          }
     }
...
int main(int argc, char **argv)
{
     ...
     for (int i = 0; i < pthreads_i; i++)
     {
          ...
          {
               printf("Thread join failed for thread: %s [%d]\n", strerror(pthread_join_result), pthread_join_result);
          }
     }
